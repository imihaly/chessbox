...
int main(int argc, const char * argv[]) {
    ...
    BENCHMARK(std::string("allMoves: ") + fen, 150000) {
    ...
    BENCHMARK(std::string("allMoves: ") + fen, 2500000) {
        for(int idx = 0; idx < count; idx++) {
            moves.clear();
            Movegen::allMoves(position, moves);
        }
    ...
    position = FEN::positionFromFEN(fen);
    BENCHMARK(std::string("allMoves: ") + fen, 250000) {
        for(int idx = 0; idx < count; idx++) {
            moves.clear();
            Movegen::allMoves(position, moves);
        }
    ...
    position = FEN::positionFromFEN(fen);
    BENCHMARK(std::string("allMoves: ") + fen, 2200000) {
        for(int idx = 0; idx < count; idx++) {
            moves.clear();
            Movegen::allMoves(position, moves);
        }
